...
| S1 · Spatial expedition — implemented | Addressable galactic fields, system and moon hierarchy, geographic view navigation, geographic site selection and persistent regional industry. Surface camera controls remain usable while paused. | Resolve a field without moving the ship, approach a particular planet, establish two geographic footholds and return to the same factories after saving. |
| S1b · Physical space — implemented | Persistent astronomical scene and camera, physical radii/separation, mass-dependent orbital periods, body-fixed geography and matching ground sunlight. Shared fullscreen controls, including Mac. | Approach a planet without replacing its mesh; resolve its moon at the true separation; overlay regions without resetting camera or rotation; preserve factories and saves. |
...
[Spatial expedition](spatial-expedition.md) removes the one-surface-per-body assumption. It adds lazy stellar catalogue fields, a real system hierarchy, regional coordinates and an inspectable descent. These are foundations for continuous worlds, not a completed galaxy simulator. The active industrial simulation is still an 80 m worksite and the wider planetary globe remains a reconstruction. The [physical space pass](physical-space.md) now supplies an astronomical coordinate frame and a persistent camera instead of screen-image transitions. The next spatial milestone must tackle the remaining ground boundary directly, with chunk streaming and industrial expansion across chunks.

Day/night power and storage must follow the same illumination model before visual night can govern industrial output. Preserve deterministic long-absence integration when replacing the current mean-exposure budget.
