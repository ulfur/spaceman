...
[Prospects](prospects.md) now extends that interaction to six generated candidate systems. Distant observing programmes supply incomplete evidence; local probes unlock new surface regions whose solar output and deposits follow the measured planet. The player can choose an expedition, build industry and return under shared rules. Full environmental evolution and planetary interventions follow in later milestones. The existing authored journey remains useful as a regression fixture and a test of autonomy.
...
[Testbeds](testbeds.md) implements the first bounded environmental intervention: build a chamber from surface output, manage heat, gas inventory, water and shielding, and observe the response of an archive culture. Its measured history persists through departure and return. The next extension must take environmental engineering beyond the enclosure while preserving meaningful mass, energy and time budgets.
