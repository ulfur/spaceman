...
On macOS, the engine reports its requested fullscreen mode before Cocoa finishes the transition. The controller now holds subsequent toggles until the native titlebar/resize callbacks arrive. This prevents a late completion callback from overwriting a newer exit request. The behavior follows Godot 4.5's [macOS display backend](https://github.com/godotengine/godot/blob/4.5-stable/platform/macos/display_server_macos.mm) and [window delegate](https://github.com/godotengine/godot/blob/4.5-stable/platform/macos/godot_window_delegate.mm). The native test verifies actual menu opening, restoration of the original desktop mode, and absence of a stray fullscreen error dialog.
