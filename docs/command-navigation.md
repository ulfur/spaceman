...
On macOS, the engine reports its requested fullscreen mode before Cocoa finishes the transition. The controller now holds subsequent toggles until the native titlebar/resize callbacks arrive. This prevents a late completion callback from overwriting a newer exit request. The behavior follows Godot 4.5's [macOS display backend](https://github.com/godotengine/godot/blob/4.5-stable/platform/macos/display_server_macos.mm) and [window delegate](https://github.com/godotengine/godot/blob/4.5-stable/platform/macos/godot_window_delegate.mm). The native test verifies actual menu opening, return to a desktop window, and absence of a stray fullscreen error dialog. The backend can classify a restored desktop window as maximized when its frame fills the usable screen; that is distinct from remaining in fullscreen.
