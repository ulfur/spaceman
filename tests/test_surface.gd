...
func _initialize() -> void:
	...
	var restored_session: Dictionary = session_copy.restore_json(full_save)
	check(restored_session.ok, "Complete expedition plus site restored: " + restored_session.message)
	...
	var migrated: Dictionary = session_copy.restore_json(load("res://scripts/simulation.gd").new().save_json())
	check(migrated.ok and session_copy.sites.is_empty(), "Original version-one expedition saves migrate: " + migrated.message)
