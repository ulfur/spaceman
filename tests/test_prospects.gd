...
func _initialize() -> void:
	...
	check(restored.restore_json(saved).ok, "Version-three expedition restores in a generated system")
	...
	check(restored.restore_json(JSON.stringify(old_v2)).ok, "First Foothold version-two save migrates")
