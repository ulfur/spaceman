...
func build_interface() -> void:
	...
	map_title.add_child(UI.label("STELLAR NAVIGATION", 26))
	map_title.add_child(UI.label("Select a star · Double-click to resolve · Drag to pan", 15, MUTED))
	var rail := UI.row(UI.mount(self, Control.PRESET_BOTTOM_WIDE, Vector4(32, -174, -414, -124)))
	rail.add_child(UI.button("−", func(): map.zoom_by(-0.5), "NavZoomOut"))
	rail.add_child(UI.button("+", func(): map.zoom_by(0.5), "NavZoomIn"))
	rail.add_child(UI.label("Scroll / pinch to zoom · Middle / right drag to pan", 13, MUTED))
